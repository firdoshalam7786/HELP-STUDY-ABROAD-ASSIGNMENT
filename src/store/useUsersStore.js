...
const useUsersStore = create((set, get) => ({
  ...
  fetchUsers: async () => {
    ...
    try {
      ...
      const fetchSearch = async (q) => {
        const url = `https://dummyjson.com/users/search?q=${encodeURIComponent(q)}&limit=${limit}&skip=${skip}`;
        const r = await axios.get(url, { timeout: 10000 });
        return {
          users: Array.isArray(r.data?.users) ? r.data.users : [],
          total: typeof r.data?.total === "number" ? r.data.total : (Array.isArray(r.data?.users) ? r.data.users.length : 0)
        };
      };
      ...
      if (qRaw.length > 0) {
        ...
        if (usersArr.length === 0) {
          ...
          usersArr = Array.isArray(listRes.data?.users) ? listRes.data.users : [];
          totalCount = typeof listRes.data?.total === "number" ? listRes.data.total : usersArr.length;
        }
      } else {
        ...
        totalCount = typeof res.data?.total === "number" ? res.data.total : usersArr.length;
      }
  ...
  fetchUserById: async (id) => {
    try {
      if (!id) return null;
      const res = await axios.get(`https://dummyjson.com/users/${id}`, { timeout: 10000 });
